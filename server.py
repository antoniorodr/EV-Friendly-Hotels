...
class SearchForm(FlaskForm):
    name = StringField(
        "Search for a charge station", validators=[DataRequired(), Length(1, 40)]
    )
...
class TokenForm(FlaskForm):
    email = StringField(
        "Write your email and click submit to get the token",
        validators=[DataRequired(), Length(7, 70)],
    )
    submit = SubmitField("Submit")
...
def all_chargers():
    ...
    for charger in all_relevant_chargers:
        folium.Marker([charger.latitude, charger.longitude], popup=charger.name).add_to(
            my_map
        )
...
def search():
    ...
    if form.validate_on_submit():
        ...
        result = db.session.execute(
            db.select(Charger).where(Charger.name.ilike(pattern))
        )
...
def get_token():
    ...
    if form.validate_on_submit():
        ...
        check_user = db.session.execute(
            db.select(Token).where(Token.email == form.email.data)
        )
        user = check_user.scalar()
        if user:
            flash(
                Markup(
                    "You have already received a token. Please check your email. If you can't find it or don't remember it, feel free to get in touch through: <a href='mailto:pythoncodingacc69@gmail.com'>pythoncodingacc69@gmail.com</a>"
                ),
                "info",
            )
        if result and not user:
            token = generate_token()
            new_token = Token(token=token, email=form.email.data)
            db.session.add(new_token)
            db.session.commit()
            with smtplib.SMTP(os.environ["SMTP_ADDRESS"], port=587) as connection:
                connection.starttls()
                connection.login(
                    user=os.environ["email"], password=os.environ["password"]
                )
                connection.sendmail(
                ...
                flash(
                    "An email with your token has been sent. Check your inbox. If you haven't received an email with the token, please check your junk mail folder."
                )
...
def new_charger():
    types = [
        "Hotels and accomodation, charging on private premises",
        "Charging only (no accomodation)",
        "Hotel, charging station nearby",
        "Superchargers",
        "Tesla Destination Chargers",
        "Hotel @ Supercharger",
    ]
    token = request.headers.get("token")
    check_token_result = db.session.execute(
        db.select(Token).where(Token.token == token)
    )
    check_token = check_token_result.scalars().first()
    type_ = request.form.get("type")
    if type_ not in types:
        return jsonify(
            error={"message": f"The type should be one of the follow: {types}"}
        ), 400
    longitude = request.form.get("longitude")
    latitude = request.form.get("latitude")
    existing_charger = (
        db.session.query(Charger)
        .filter_by(longitude=longitude, latitude=latitude)
        .first()
    )
    if existing_charger:
        return jsonify(
            error={"message": "A charger with this coordinates already exists."}
        ), 400
    if check_token:
        new_charger = Charger(
...
def search_cafe():
    ...
    if check_token:
        ...
        if all_chargers:
            ...
            return jsonify(
                error={"Not Found": "Sorry, we don't find a charger at that location."}
            ), 404
    return jsonify(error={"error": "You need a valid token."}), 401
...
def update(charger_id):
    ...
    if check_token:
        if result:
            ...
            db.session.commit()
            return jsonify(
                response={"success": "Successfully updated the description."}
            ), 200
        else:
            return jsonify(
                error={
                    "Not Found": "Sorry, a charger with that ID was not found in the database."
                }
            ), 404
    return jsonify(error={"error": "You need a valid token."}), 401
