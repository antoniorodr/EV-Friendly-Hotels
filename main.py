...
def extract_kml():
    ...
    extraction_dir = os.path.dirname("data/EV-friendly hotels in Europe.kmz")
    fiona.drvsupport.supported_drivers["libkml"] = "rw" 
    fiona.drvsupport.supported_drivers["LIBKML"] = "rw"
    with zipfile.ZipFile(kmz_file_path, "r") as kmz:
        kmz.extractall(extraction_dir)

def get_kml_layers(kml_file):
    layers_list = []
...
def create_csv(func_get_kml_layers):
...
if __name__ == "__main__":
    extract_kml()
    create_csv((get_kml_layers(KML_FILE)))